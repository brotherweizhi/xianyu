                ...
                <div class="am-btn-toolbar">
                    <div class="am-btn-group am-btn-group-xs">
                        <button type="button" class="am-btn am-btn-default"
                                onClick="location.href='/Admin/User/../user/add'"><span class="am-icon-plus"></span> 新增
                        </button>
                ...
                <form class="am-form">
                    <table class="am-table am-table-striped am-table-hover table-main">
                        ...
                        <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td class="am-hide-sm-only"><?php if(!empty($vo["created"])): echo (date('Y-m-d
                                H:i',$vo["created"])); endif; ?>
                            </td>
                            <td>
                                <div class="am-btn-toolbar">
                                    <div class="am-btn-group am-btn-group-xs">
                                        <button type="button" class="am-btn am-btn-default am-btn-xs am-text-secondary"
                                                onClick="location.href='/Admin/User/../user/edit?uid=<?php echo ($vo["uid"]); ?>'"><span
                                                class="am-icon-pencil-square-o"></span> 编辑
                                        </button>
                                        <button type="button"
                                                class="am-btn am-btn-default am-btn-xs am-text-danger am-hide-sm-only"
                                                onclick="if (confirm('确定?')) location.href='/Admin/User/../user/del?uid=<?php echo ($vo["uid"]); ?>'">
                                            <span class="am-icon-trash-o"></span> 删除
                                        </button>
                                        <button type="button" class="am-btn am-btn-default am-btn-xs  am-hide-sm-only"
                                                onclick="location.href='/Admin/User/../user/status?uid=<?php echo ($vo["uid"]); ?>&status=<?php echo ($vo["status"]); ?>'">
                                            <span class=""></span> <?php if(($vo["status"]) == "0"): ?>正常<?php else: ?>已封禁<?php endif; ?>
                                        </button>
...
<style>
    ...
    DIV.digg A:hover {
        BORDER-RIGHT: #0E90D2 1px solid;
        BORDER-TOP: #0E90D2 1px solid;
        BORDER-LEFT: #0E90D2 1px solid;
        COLOR: #000;
